getCond <- function(startDt, endDt, stationdata) {
  ...
  for (i in 1:numStations) {
    ...
    if (nrow(conditionData) > 0) {
      naRows <- which((conditionData[, "tempC"] < -100)
                      | (conditionData[, "tempC"] > 100)
                      | (conditionData[, "humPer"] <= 0)
                      | (conditionData[, "humPer"] > 100)
                      | (conditionData[, "windKm"] < 0)
                      | (conditionData[, "wdirDeg"] < 0)
                      | (conditionData[, "wdirDeg"] > 360)
                      | (conditionData[, "pressureMb"] < 0)
                      | (conditionData[, "precip_rateMm"] < 0))
      if (length(naRows) > 0) {conditionData[naRows,] <- NA}
    }   
